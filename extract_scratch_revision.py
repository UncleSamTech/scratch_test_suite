...
def get_revisions_and_run_parser(cwd, main_branch,project_name,  debug=False):
    ...
    # for all sb3 files in ths project
        for f in filenames:
            ...
            proc1 = subprocess.run(['git --no-pager log -z --numstat --follow --pretty=tformat:"{}¬%H" -- "{}"'.format(escape_special_chars(f),escape_special_chars(f))], stdout=subprocess.PIPE, cwd=cwd, shell=True)
