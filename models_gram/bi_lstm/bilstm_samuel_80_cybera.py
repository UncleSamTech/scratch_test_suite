...
class bilstm_cybera:
    ...
    def evaluate_bilstm_in_order_optimized2(self, test_data_path, maxlen, model_path, result_path, proj_number, run, logs_path):
        ...
        try:
            ...
            test_data_files = [
                f"{test_data_path}/scratch_test_set_{proj_number}_6_{run}_proc_{i}.txt"
                for i in range(1, 51)
            ]
...
sample = ("/mnt/siwuchuk/vscode/output_train/tmp80", "/mnt/siwuchuk/vscode/models/bilstm/model/80/", "/mnt/siwuchuk/vscode/output_test/80", 80, "/mnt/siwuchuk/vscode/models/bilstm/logs/80")
cl_ob.consolidate_data_train_parallel(*sample)
#split -l $(( ($(wc -l < scratch_test_set_50_6_1_proc.txt) + 3) / 10)) scratch_test_set_50_6_1_proc.txt scratch_test_set_50_6_1_proc_ && for f in scratch_test_set_50_6_1_proc_*; do mv "$f" "${f}.txt"; done