...
def run_on_core():
    ...
    all_cores = set(range(multiprocessing.cpu_count()))  # Get all cores
    used_cores = tr_scr.get_used_cores()  # Get currently occupied cores
    ...
    if not available_cores:
        ...
        while not available_cores:
            ...
            used_cores = tr_scr.get_used_cores()
            available_cores = list(all_cores - used_cores)

    # Start processes and assign them to free cores
    processes = []
    for i, data in enumerate(datasets):

        if i < len(available_cores):  # Ensure we don’t exceed available cores
            tr_scr.multiple_train_time_metrics_new(datasets[i][0], datasets[i][1],datasets[i][2],datasets[i][3],datasets[i][4],datasets[i][5])
            core = available_cores[i]
            p = multiprocessing.Process(target=run_on_core, args=(*data, core))
            p.start()
            processes.append(p)
